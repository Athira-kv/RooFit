#define phi_roofit_mom_cxx
#include "phi_roofit_mom.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
  ...
  if (fChain == 0) return;
   ...
   for (int p = 0; p < 5; ++p) {
     for (size_t i = 0; i < mombins.size(); ++i) {
       ...
       TString histName = Form("phi_invMass_%d_for_mom_%.2f", p, min_p);
   ...
   TFile *outputFile = new TFile("phi_input.root", "RECREATE");
