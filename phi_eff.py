...
data = np.loadtxt('phi_signal_background_results_simfit.txt', skiprows=1)
...
plt.ylabel(r'$\epsilon(K^+ \rightarrow j^+) = \frac{N_{K^+ \rightarrow j^+}^{Sig}}{N_{{All events}}^{Sig}}$', fontsize=18)
plt.legend()
plt.grid()
