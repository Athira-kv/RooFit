...
#ifndef k_roofit_mom_h
#define k_roofit_mom_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   k_roofit_mom(TTree *tree=0);
   virtual ~k_roofit_mom();
...
#ifdef k_roofit_mom_cxx
k_roofit_mom::k_roofit_mom(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../k0_uDST.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../k0_uDST.root");
      }
...
k_roofit_mom::~k_roofit_mom()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t k_roofit_mom::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t k_roofit_mom::LoadTree(Long64_t entry)
{
...
void k_roofit_mom::Init(TTree *tree)
{
...
void k_roofit_mom::Show(Long64_t entry)
{
...
Int_t k_roofit_mom::Cut(Long64_t entry)
{
