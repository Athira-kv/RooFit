...
#ifdef kaon_fit_cxx
kaon_fit::kaon_fit(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../W8t4_k0_mDST.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../W8t4_k0_mDST.root");
      }
...
Int_t kaon_fit::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t kaon_fit::LoadTree(Long64_t entry)
{
...
void kaon_fit::Show(Long64_t entry)
{
...
Int_t kaon_fit::Cut(Long64_t entry)
{
